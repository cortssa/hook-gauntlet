<!-- selftest: judge=5=false rows=7 rc=0 -->
# STATE - next.sh fixture: row 7 - the long fuzz has not run on this bytecode

*A fixture for scripts/next.sh (scripts/selftest.sh). The first line says how it is run and what it must give.*

```
phase:                     4
bytecode_changed_since:    last_battery=no  last_long_fuzz=yes  last_other_free_judges=no  last_audit_round=yes  last_promotion=n/a
battery:                   green
blackbox:                  current
