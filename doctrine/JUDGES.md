...
| 4 | Is the campaign reaching everything? | the campaign census (`scripts/census.sh`: runs in which each action succeeded, each boundary was reached; `CORE` and `REACH` make the ones that matter a gate), `forge coverage --report summary --no-match-coverage '^([^st]|s($|[^r])|sr($|[^c])|src($|[^/])|t($|[^e])|te($|[^s])|tes($|[^t])|test($|[^/]))'` by **branch** (the regex keeps only your `src/` and `test/` in the totals, wherever the kit and forge-std live - forge reports remapped files by the path they came in on, and 1.8.1 has no `--match-coverage`; `foundry-kit/v4/README.md`, coverage; if it will not compile: `--ir-minimum`; a hook compiled next to the PoolManager's IR restriction (the kit's v4 module, `QUICKSTART.md` 7b): `--ir-minimum` ALWAYS - without it forge 1.8.1 measures the wrong build (every test that creates the PoolManager is compiled only under the restricted via-IR profile, so the hook it deploys is the optimised `.manager` build, which coverage maps to the wrong lines: lines of a `claim` at 0 that every claim test runs, seen on a stranger's hook); with `--ir-minimum` there is one build and the numbers agreed with execution - before citing a number, check that a function a test runs whole has the same count on every line; if that will not either: "not done", with the compiler error) | yes - branch numbers, or the reason there are none |
