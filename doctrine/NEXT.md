...
| 1 | `waiting_on_owner` is not `none` | nothing that depends on the answer. **This row is not terminal and does not outrank row 3**: check row 3, then continue down the table for work that does not depend on the answer; stop only if there is none | the owner's decisions are the owner's - and a high finding does not wait behind another question |
| 2 | the ceiling is reached | STOP and report: what was bought, what is open, what the next unit would cost | a route without a ceiling does not end |
| 3 | a **high** finding reproduces and the owner has not been told | STOP and tell them now, with the test | never batch a high |
...
| 14 | `last_audit_round` was a **discovery** round (no earlier reports in its brief) and closed with 0 high and 0 medium, no REASONED high or medium is open (`EVIDENCE.md` 3), AND no bytecode changed since | the loop is over. Go to row 15 | the exit criterion is the stop rule; no "one more to be safe" |
...
- **If you take the same row twice in a row and nothing changed in between, STOP.** You forgot to update a flag, or
  the table has a hole. Say which, in `STATE.md`, and ask.
