...
| 6b | `battery` is `red` | make it green: fix the code or the test, at the cause. **No model round of any kind runs on a red battery.** One exception, and it is a finding, not a fix: a hostile-token ACTION (`FUZZ-ACTIONS.md` 3) breaks a promise the owner has not decided on - then open the finding (its id, its test seen red), move THAT invariant or action to `pending/<id>.t.sol` OUTSIDE `test/` - forge compiles only the directories it is pointed at, so the battery is honestly green - and run it on purpose with `FOUNDRY_PROFILE=pending forge test --match-path 'pending/*'` from a `[profile.pending]` whose `test = "pending"` (the same mechanism the sandbox uses for `sim/`; without `--match-path` forge also runs the base suite the pending file imports, a whole extra campaign). Then delete `cache/invariant/failures/<suite>/` for the everyday suite: that counterexample is now the pending test's red, and forge would replay it into the battery. Give the finding a PROVISIONAL severity on `SEVERITY.md` (marked `provisional`, the owner's triage sets it); a provisional high is row 1's, write `pending: <id> - <promise>, owner undecided` in `STATE.md` `notes:`, and the round brief names it. Never silently leave the hostile action out | an auditor's first act is to reproduce the baseline; a red one means they are attacking something else |
