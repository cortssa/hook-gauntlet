...
| 3 | `waiting_on_owner` is not `none` | skip every row below whose action depends on the answer, take the first that does not; if none, stop and say what is waiting. An undecided light/full mode does not block round 1: light is assumed WITH its ceiling from `COST.md` §1 (3 model rounds + 1 black-box = 4), written in `DECISIONS.md` with `source: assumed, owner absent` | the owner's decisions are the owner's |
...
| 6b | `battery` is `red` | make it green: fix the code or the test, at the cause. **No model round of any kind runs on a red battery.** One exception, and it is a finding, not a fix: a hostile-token ACTION (`FUZZ-ACTIONS.md` 3) breaks a promise the owner has not decided on - then open the finding (its id, its test seen red), move THAT invariant or action to `pending/<id>.t.sol` OUTSIDE `test/` - forge compiles only the directories it is pointed at, so the battery is honestly green - and run it on purpose with `FOUNDRY_PROFILE=pending forge test` from a `[profile.pending]` whose `test = "pending"` (the same mechanism the sandbox uses for `sim/`), write `pending: <id> - <promise>, owner undecided` in `STATE.md` `notes:`, and the round brief names it. Never silently leave the hostile action out | an auditor's first act is to reproduce the baseline; a red one means they are attacking something else |
