...
| initialise a **native-currency** pool (r01, F2) | accepted: the hook moves no value and reads no currency, so nothing changes | `test_r01_F2_a_native_currency_pool_is_accepted_and_charges_its_quote` (a real swap each way; a mutant that refuses native pools is KILLED by it) | - (run through v4-core's `PoolSwapTest`; the kit's own router accepts native pools since 2026-09-24) |
